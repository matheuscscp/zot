...
type AccessController struct {
	...
	Log    log.Logger
	// compiled is a snapshot of the Controller's compiled-conditions map taken
	// at construction time. Snapshotting (rather than chasing the live atomic
	// pointer per lookup) keeps a single request consistent if a hot reload
	// fires mid-evaluation.
	compiled map[string]*cel.Expression
}

func NewAccessController(ctlr *Controller) *AccessController {
	// Get access control config safely
	accessControlConfig := ctlr.Config.CopyAccessControlConfig()
	logConfig := ctlr.Config.CopyLogConfig()

	var compiled map[string]*cel.Expression
	if p := ctlr.compiledConditions.Load(); p != nil {
		compiled = *p
	}
...
func (ac *AccessController) getGlobPatterns(evalReq *evalRequest) map[string]bool {
	...
	for pattern, policyGroup := range ac.Config.Repositories {
		...
		for _, policy := range policyGroup.Policies {
			...
			if ok, _ := ac.policyConditionsMet(policy, evalReq); ok {
				globPatterns[pattern] = true
			}
		}
...
func CompileAccessControl(cfg *config.AccessControlConfig) (map[string]*cel.Expression, error) {
	if cfg == nil {
		return nil, nil //nolint:nilnil // empty cfg = no programs to compile
	}

	programs := map[string]*cel.Expression{}
...
// AccessController's snapshot. The expression must have been registered by
// CompileAccessControl; otherwise authorization fails closed.
func (ac *AccessController) lookupCondition(expr string) (*cel.Expression, error) {
	if program, ok := ac.compiled[expr]; ok {
		return program, nil
	}
	...
	return func(next http.Handler) http.Handler {
		return http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
			...
			*/
			...
			aCtlr := NewAccessController(ctlr)
